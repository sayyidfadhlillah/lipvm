...
(() => {
  ...
  // src/model.ts
  var Command = class {
    constructor(store, migrationstatements, statements) {
      this.store = store;
...
return true;`);
  ...
  var Store = class {
    async init() {
      ...
      const logger = new P();
      this.db = new O(logger, this.worker);
    ...
    async executeCountQuery(st) {
      const con = await this.db?.connect();
      const s = await con?.query(st);
      await con?.close();
      return s;
    }
    async close() {
