...
from citylearn.energy_model import HeatPump as CityLearnHeatPump
...
class HeatPumpSnapshot:
    ...
    CityLearnHeatPump already has that name as a full time-series array
    property; reusing it here would shadow it (see HeatPump's own docstring
    below)."""

    current_electricity_consumption: float


class HeatPump(CityLearnHeatPump, PartSimulationModel):
    """Extends `citylearn.energy_model.HeatPump` directly alongside
    ...
    snapshot_type = HeatPumpSnapshot
